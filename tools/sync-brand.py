...
Course Factory does not carry an identity of its own. It publishes with the
...
It never writes to the source. It never touches course material.
"""
...
def do_check(brand_path: str) -> int:
    ...
    if not brand.exists():
        ...
        print()
        print("  Full contract: docs/03-brand-kit.md")
        return 1
    gaps = brand.missing()
    if gaps:
        print("Contract NOT satisfied. Missing: %s" % ", ".join(gaps))
        print("See docs/03-brand-kit.md.")
        return 1
...
def do_from_template(dry_run: bool, force: bool) -> int:
    ...
    if not dry_run:
        ...
        print("  python3 tools/publish-course.py <subject>   # and look at the PDF")
        print()
        print("Full contract and checklist: docs/03-brand-kit.md")
    return 0
...
def do_colors(brand_path: str) -> int:
    ...
    if not declared:
        ...
        print("      BRAND_COLOR_<name>=RRGGBB      # see docs/03-brand-kit.md")
    else:
...
def do_palette(brand_path: str, dry_run: bool) -> int:
    ...
    if not rendered:
        ...
        print("Declare the colours there first; see docs/03-brand-kit.md.")
        return 1
...
def report(tally: dict, dry_run: bool, what: str, source: Path,
    ...
    if dry_run or next_step != "republish":
        return
    # Telling someone to republish courses they have not written yet is noise;
    # only the paths that change an existing identity say this.
    print()
    print("Republish the courses so the change reaches the PDFs:")
    print("  python3 tools/publish-all.py courses --jobs 6")
